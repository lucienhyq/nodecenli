...
let checkLogin = function (req) {
  if (req.session.user) {
    mongo("find", "user", { userName: req.session.user.userName }, function (
      data
    ) {
      console.log(data);
    });
    ...
    console.log(req.session.user, "检查session");
    return { result: 0, msg: "请登录" };
  }
};
// 注册接口
